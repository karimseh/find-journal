...
def get_stats(conn: sqlite3.Connection) -> dict:
    ...
    enriched = conn.execute('SELECT COUNT(*) FROM journals WHERE quartile IS NOT NULL').fetchone()[0]
    ...
    top_publishers = [{'publisher': row['publisher'], 'count': row['cnt']} for row in publisher_rows]
